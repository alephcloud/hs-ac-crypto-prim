...
static int curve25519_random_fd = -1;

static inline void curve25519_randombytes(unsigned char *x,unsigned long long xlen)
{
  int i;

  if (curve25519_random_fd == -1) {
    for (;;) {
      curve25519_random_fd = open("/dev/urandom",O_RDONLY);
      if (curve25519_random_fd != -1) break;
      sleep(1);
    }
  }
  ...
  while (xlen > 0) {
    ...
    i = read(curve25519_random_fd,x,i);
    if (i < 1) {
      sleep(1);
      continue;
    }
...
static inline void curve25519_select(unsigned int p[64],unsigned int q[64],const unsigned int r[64],const unsigned int s[64],unsigned int b)
{
  unsigned int j;
  unsigned int t;
...
static inline void recip(unsigned int out[32],const unsigned int z[32])
{
  unsigned int z2[32];
  unsigned int z9[32];
  unsigned int z11[32];
